...
void norep(void); /* sorteia um numero aleatorio */
...
int main(void)
{
    ...
    for(j=0; j<MAX; j++)
    {
        quantidade[j] = 0; /* inicialmente nenhum numero foi sorteado ainda, por isso a atribuicao de zero */
    }
    ...
    printf("\n");
    for(j=0; j<MAX; j++)
        somatorio += quantidade[j]; /* faz o somatorio da quantidade de sorteios de todos os numeros */
    for(j=0; j<MAX; j++)
    {
        p = (quantidade[j]*100)/somatorio;
        printf("O numero %d foi sorteado %.0f vezes (%f%%)\n", j, quantidade[j], p); /* mostra quantas vezes cada numero foi sorteado e a porcentagem */
    }
    return 0;
}
